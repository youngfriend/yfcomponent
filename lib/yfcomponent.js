...
exports.createTpl = function (args) {
    ...
    dirs.forEach(function (value, index) {
        var suffix, tplFileName = params[1], dir = path.join(root, value);
        switch (index) {
            case 0:
                suffix = '.less';
                break;
        ...
        try {
            fs.createReadStream(path.join(__dirname, '../template', value, 'temp' + suffix)).pipe(fs.createWriteStream(path.join(dir, tplFileName + suffix)));
        } catch (err) {
            console.log(err);
        }
    });
